...
from buey_robot.contracts import GPS_FIX, HEADING_FUSED, ODOM, GEO_POSITION, GEO_ROUTE, LOCAL_ROUTE

PARAMS = {
    'allow_float': ('allow_float', bool),
    'fixed_max_accuracy': ('rtk.fixed_max_accuracy_m', float),
    'float_max_accuracy': ('rtk.float_max_accuracy_m', float),
    'filter_enabled': ('filter.enabled', bool),
    'filter_window': ('filter.window_size', int),
    'antenna_offset': ('robot.antenna.offset_x_m', float),   # de robot.yaml (compartido)
...
class OdometryGps(Node):
    def __init__(self):
        ...
        self._acc_log = TransitionLogger(self.get_logger())   # gate de accuracy, solo en transiciones

        # Salidas
        self._odom_pub = self.create_publisher(Odometry, ODOM, 10)
        self._geo_pub = self.create_publisher(String, GEO_POSITION, 10)
    ...
    def _on_fix(self, msg):
        ...
        if acc > self._max_accuracy():        # fix poco preciso -> no publico odometria
            self._acc_log.warn(
                ...
                f'({"RTK Float" if self.allow_float else "RTK Fixed"}). No publico /odom.', key=False)
            return
        self._acc_log.info(f'GPS confiable (accuracy {acc:.2f}m) -> publicando /odom', key=True)
        ...
        lat, lon = self._conv.local_to_gps(x, y)   # centro del robot en lat/lon (para el mapa)
        self._geo_pub.publish(String(data=json.dumps({'lat': lat, 'lon': lon})))
